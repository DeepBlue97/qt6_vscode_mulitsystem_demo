    ...
    , ui(new Ui::MainWindow)
{
    ...
    /*graphicsView = new QGraphicsView(ui->centralwidget);
    graphicsView->setObjectName("graphicsView");

//    gridLayout->addWidget(graphicsView, 0, 0, 1, 1);

//    QGraphicsView* graphicsView = new(QGraphicsView);
    ui->gridLayout->addLayout(graphicsView, 0, 0, 1, 1);*/

    this->setWindowTitle(QString("QGraphicsItem基本图元演示 - V%1").arg("1.0.0"));
...
void MainWindow::initToolBar()
{
    ...
    QPushButton* but_scale = new QPushButton("整体缩放");
    QPushButton* but_move = new QPushButton("整体移动");
    ...
    but_scale->setCheckable(true);
    but_move->setCheckable(true);
    ...
    m_butGroup.addButton(but_scale);
    m_butGroup.addButton(but_move);
    ...
    ui->toolBar->addWidget(but_scale);
    ui->toolBar->addWidget(but_move);
    ...
    connect(but_scale, &QPushButton::clicked, this, &MainWindow::scale);
    connect(but_move, &QPushButton::clicked, this, &MainWindow::move);
...
//    parentItem->setPos(200, 200); // 设置父级位置

    // 将图像项添加到父级项中
//    parentItem->addToGroup(item);

    // 在场景中添加父级项
//    m_scene.addItem(parentItem);
    item->setPos(200, -100);
...
//    QPointF p( 240., 89.1);
//    item->setPos(p);
//    item->setOffset(-300, 0);
    //    m_scene->show();
//    std::cout << "setPos done" << std::endl;
//    QPointF p( 40., 89.1);
    qInfo() << "offset: " << item->offset();
    qInfo() << "pos: " << item->pos();
...
void MainWindow::drawPath()
{
    ...
    // 圆角矩形
    path.addRoundedRect(QRect(20, 150, 100, 100), 20, 20);
